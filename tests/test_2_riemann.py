...
def test_expansion_shock():
    ...
    assert pb.left_fastest() == pytest.approx(qL.left_acoustic(), rel=1e-10) # expansion has C- from qL
...
def test_expansion_expansion():
    ...
    pb = riem.riemann_pb(qL, qR)
    assert pb.left_fastest() == pytest.approx(qL.left_acoustic(), rel=1e-10) # expansion has C- from qL
    assert pb.right_fastest() == pytest.approx(qR.right_acoustic(), rel=1e-10) # expansion has C+ from qR
...
def test_expansion_expansion_shift():
    ...
    assert pb2.pstar() == pytest.approx(pb1.pstar(), rel=1e-10) 
    assert pb2.left_fastest()-pb1.left_fastest() == pytest.approx(1., rel=1e-10) # shift u = 1
    assert pb2.right_fastest()-pb1.right_fastest() == pytest.approx(1., rel=1e-10) 
...
def test_shock_shock():
    ...
    assert pb.right_fastest() > qR.right_acoustic() # shock wave is faster than qR C+
    for Ws, q0, q1 in ((pb.left_fastest(), qL, pb.qstarL()),(pb.right_fastest(), qR, pb.qstarR())):
        # mass balance
