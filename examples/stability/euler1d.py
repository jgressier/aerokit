...
class Tube(Euler1D):
    def __init__(self, n, Mach) -> None:
        ones = np.ones(n)
        asound = 100.0
        self.Q = state(rho=1.4 * ones, u=(Mach * asound) * ones, p=asound**2.0 * ones)
        super().__init__(n, xmin=0.0, xmax=1.0, basestate=self.Q)
        self.set_BC("per", "per")
...
n = 101
model = Tube(n, Mach=0.55)
model.solve_eig()
Aomega, Avects, order = model.select_and_sort(0.01, 5000.0, -1.0, 1.0)
...
plt.plot(Aomega.real, Aomega.imag, "ob", markersize=10, alpha=0.2)
...
for i in range(nvec):
    ...
    phase = np.average(
        np.mod(np.angle(v1) - np.angle(v0), 2 * np.pi), weights=np.abs(v1)
    )
