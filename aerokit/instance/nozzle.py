...
def Ms_from_AsAc_NPR(AsAc, NPR):
    ...
    if NPR < NPR0:
        ...
        gmu = defg._gamma - 1.0
        K = NPR / AsAc / ((defg._gamma + 1.0) / 2) ** ((defg._gamma + 1.0) / 2 / gmu)
        Ms = np.sqrt((np.sqrt(1.0 + 2.0 * gmu * K * K) - 1.0) / gmu)
    return Ms
...
class nozzle:
    ...
    def __init__(self, x, section, AsoAc=None, gamma=1.4, NPR=None, ref_rttot=1.0, scale_ps=1.0):
