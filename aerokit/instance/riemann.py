...
class riemann_pb:
    ...
    def qsol(self, xot):
        ...
        if self._pstar < self._qL.p:
            ...
            gp1 = gam + 1.0
            gm1 = gam - 1.0
            u[i] = gm1 / gp1 * (self._qL.u + 2.0 / gm1 * (self._qL.asound() + xot[i]))
            p[i] = self._qL.p * ((u[i] - xot[i]) / self._qL.asound()) ** (2.0 * gam / gm1)
        ...
        if self._pstar < self._qR.p:
            ...
            gp1 = gam + 1.0
            gm1 = gam - 1.0
            u[i] = gm1 / gp1 * (self._qR.u + 2.0 / gm1 * (-self._qR.asound() + xot[i]))
            p[i] = self._qR.p * ((xot[i] - u[i]) / self._qR.asound()) ** (2.0 * gam / gm1)
            rho[i] = self._qR.rho_through_isentropic(p[i])
        return uq.unsteady_state(rho, u, p, np.where(xot < 0.0, self._qL._gamma, self._qR._gamma))
    ...
    def _ustar_from_pstar(self, p):
        return 0.5 * (self._qR.u + self._qL.u + self._delta_uR(p) - self._delta_uL(p))
    ...
    def _pstar_estimate_expansion(self):
        ...
        """
        gam = 0.5 * (self._qL._gamma + self._qR._gamma)
        gmusd = 0.5 * (gam - 1.0)
        return (
            (self._qL.asound() + self._qR.asound() - gmusd * (self._qR.u - self._qL.u))
            / (
                self._qL.asound() / self._qL.p ** (gmusd / gam)
                + self._qR.asound() / self._qR.p ** (gmusd / gam)
            )
        ) ** (gam / gmusd)
