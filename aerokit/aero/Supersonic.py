...
def PrandtlMeyer_Mach(Mach, gamma=defg._gamma):
    ...
    cg = np.sqrt((g + 1.0) / gm1)
    return (cg * np.arctan(beta / cg) - np.arctan(beta)) * (180.0 / math.pi)
...
def old_Mach_PrandtlMeyer(omega, gamma=defg._gamma):
    ...
    return IterativeSolve.secant_solve(omega_of_mach, omega, 2.0)
...
def Mach_PrandtlMeyer(omega, gamma=defg._gamma):
    ...
    result = newton(omega_of_mach, 2.0 + 0.0 * omega)
...
def Mach_PMFmmu(om_m_mun, gamma=defg._gamma):
    ...
    gm1 = gamma - 1.0
    cg = np.sqrt((g + 1.0) / gm1)
    return np.sqrt((cg * deg.tan((om_m_mun + 90.0) / cg)) ** 2 + 1.0)
