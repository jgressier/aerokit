...
def maxTiratio_Mach(Mach, gamma=1.4):
    ...
    """
    return 1.0 / Ti_Ticri(Mach, gamma=1.4)
...
def Pi_Picri(Mach, gamma=1.4):
    ...
    return (
        (gamma + 1)
        / (1 + gamma * m2)
        * (2 / (gamma + 1) * (1 + (gamma - 1) / 2 * m2)) ** (gamma / (gamma - 1))
    )
...
def SubMach_TiTicri(Tiratio, gamma=1.4):
    ...
    alpha = np.sqrt(1.0 - Tiratio)
    return np.sqrt((1.0 - alpha) / (alpha * gamma + 1))


def SupMach_TiTicri(Tiratio, gamma=1.4):
    """computes Mach number from Ti/Ti_cri ratio (<1)"""
    alpha = np.sqrt(1.0 - Tiratio)
    return np.sqrt((1.0 + alpha) / (1.0 - alpha * gamma))
