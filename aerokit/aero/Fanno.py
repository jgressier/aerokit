...
def Ps_Pscri(Mach, gamma=1.4):
    ...
    return 1.0 / (Mach * np.sqrt(2 / (gamma + 1) * (1 + (gamma - 1) / 2 * np.square(Mach))))
...
def Pi_Picri(Mach, gamma=1.4):
    ...
    return (
        1.0
        / Mach
        * (2 / (gamma + 1) * (1 + (gamma - 1) / 2 * np.square(Mach))) ** ((gamma + 1) / 2 / (gamma - 1))
    )
...
def NormdS(Mach, gamma=1.4):
    ...
    return np.log(
        Mach ** ((gamma - 1) / gamma)
        * (2 / (gamma + 1) * (1 + (gamma - 1) / 2 * np.square(Mach))) ** (-(gamma + 1) / 2 / gamma)
    )
